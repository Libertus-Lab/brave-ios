...
class ContentBlockerManagerTests: XCTestCase {
  ...
  func testContentBlockerManager() throws {
    ...
    Task { @MainActor in
      ...
      do {
        try await manager.compile(encodedContentRuleList: encodedContentRuleList, for: .filterList(componentId: filterListUUID))
      ...
      for generalType in ContentBlockerManager.GenericBlocklistType.allCases {
        do {
          let cachedType = try await manager.loadRuleList(for: .generic(generalType))
          XCTAssertNotNil(cachedType)
        } catch {
          XCTFail(error.localizedDescription)
        }
      ...
      do {
        try await manager.removeRuleList(for: .filterList(componentId: filterListUUID))
