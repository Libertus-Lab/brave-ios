...
class FilterListStorage: ObservableObject {
  ...
  @MainActor private var pendingDefaults: [String: Bool] = [:]
  
  /// The filter lists wrapped up so we can contain
  @MainActor @Published var filterLists: [FilterList] = []
  
  /// This is a list of all available settings.
  ///
  /// - Warning: Do not call this before we load core data
  @MainActor private(set) var allFilterListSettings: [FilterListSetting]
  ...
  @MainActor func loadFilterListSettings() {
  ...
  @MainActor func loadFilterLists(from regionalFilterLists: [AdblockFilterListCatalogEntry]) {
  ...
  @MainActor func handleUpdate(to filterList: FilterList) {
    upsertSetting(
  ...
  ///
  /// - Warning: Do not call this before we load core data
  @MainActor private func upsertSetting(uuid: String, isEnabled: Bool, componentId: String, allowCreation: Bool, order: Int) {
